...
$f=mysql_query("select * from faculty where id='".$_SESSION['uid']."'");
$rec=mysql_fetch_array($f);

if(isset($_REQUEST['Save']))
{
mysql_query("update faculty set uname='".mysql_escape_string($_REQUEST['uname'])."',
						password='".mysql_escape_string($_REQUEST['pass'])."' where id='".$_SESSION['uid']."'");		
header('location:change_faculty_pass.php');
}
		...
		<td><input  type="text" name="uname" value="<?php echo $rec[1];?>" required placeholder="Title"/></td>
							...
							<tr>
									...
									<td valign="top">
											...
											<input  type="password" name="pass" value="<?php echo $rec[3];?>" required placeholder="Title"/>
									</td>
							</tr>
							<tr>
